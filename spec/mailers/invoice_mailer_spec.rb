...
describe InvoiceMailer do
    ...
    let(:firm)        { FactoryGirl.create(:firm, name:"test", bank_account: 123, vat_number: 2123, invoice_email:"firm@test.com", invoice_email_message: "TEST MESSAGE")}
    let(:user)        { FactoryGirl.create(:user, firm:firm)}
    let(:customer)		{ FactoryGirl.create(:customer, firm:firm, email: "test@test.com")}
    let(:invoice)			{ FactoryGirl.create(:invoice, firm:firm, customer:customer, number: 3)}
    let(:file)        { File.new(firm.subdomain + '_' + invoice.number.to_s + '.pdf', "w+")}
    let(:email)       { FactoryGirl.create(:email2, address: "test@test.com", firm:firm, invoice: invoice)}
